...
const Card = ({
  ...
  card_description,
  card_date,
  navroute, // سيتم تغييره لاحقاً إلى معرف الخاصية المطلوب التنقل إليها
}) => {
  const navigate = useNavigate();

  const handleClick = () => {
    // تمرير مسار التنقل مباشرة، يمكن تعديل هذا المسار كما هو مطلوب
    navigate(navroute);
  };
  ...
  return (
    <div
      onClick={handleClick}
      className="card pt-[16px] px-[16px] pb-[24px] border-[1px] border-[#64748B] rounded-[5px] shadow-[0_6px_12px_#0000001C] cursor-pointer"
    >
      <div className="card-header grid grid-cols-2 text-[14px] text-[#4B5768]">
        <span>Topic: {card_topic}</span>
        <span className="text-right">{card_level ? card_level : ""}</span>
      </div>
      ...
      <div className="card-footer mt-[15px] flex justify-between items-center">
        ...
        <div className="flex gap-[8px]">
          <div className="share-button flex items-center border border-[#4B5768] px-[10px] py-[5px] rounded-[5px] min-h-[32px] cursor-pointer">
            <img src="/img/shareicon.svg" className="mr-[8px]" alt="Share Icon" />
